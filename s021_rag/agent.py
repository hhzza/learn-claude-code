...
from rag.loader import load_directory as rag_load_directory
from rag.loader import load_file as rag_load_file
from rag.chunker import chunk_documents as rag_chunk_documents
...
PROMPT_SECTIONS = {
    ...
    "tools": "Available tools: bash, read_file, write_file, edit_file, glob, "
             ...
             "connect_mcp, "
             "search_knowledge (query the RAG knowledge base — use before answering "
             "questions about indexed documents), "
             "index_documents (build the search index from local files). "
...
def assemble_system_prompt(context: dict) -> str:
    ...
    knowledge_index = _read_knowledge_index()
    if knowledge_index:
        sections.append(
...
BUILTIN_TOOLS = [
    ...
    {"name": "search_knowledge",
     "description": "Search the RAG knowledge base with hybrid BM25+vector retrieval. "
                    "Use this when you need to find information from indexed documents "
                    "(PDFs, markdown, code). Returns top-K relevant chunks with source references.",
     "input_schema": {"type": "object",
                      "properties": {
                          "query": {"type": "string",
                                    "description": "Search query. Use specific technical terms for best results."},
    ...
    {"name": "index_documents",
     "description": "Index documents into the RAG knowledge base. "
                    ...
                    "When given a specific file path, index that file. "
                    "When given a directory, scan with glob patterns.",
     "input_schema": {"type": "object",
                      "properties": {
                          "path": {"type": "string",
                                   "description": "Directory path containing documents to index (default: current dir)"},
                          "patterns": {"type": "string",
                                       "description": "Comma-separated glob patterns (default: '*.pdf')"},
                      },
                      "required": []}},
]

# ── RAG tool handlers (defined before BUILTIN_HANDLERS so the dict can reference them) ──

def run_search_knowledge(query: str, top_k: int = 5) -> str:
    """Search the knowledge base with hybrid BM25+vector retrieval."""
    if not _rag_ready or _rag_retriever is None:
        return "Knowledge base not available. Use index_documents first."
    try:
        results = _rag_retriever.search(query, k=top_k)
    except Exception as e:
    ...
    for i, r in enumerate(results):
        lines.append(
            f"[{i + 1}] {r['heading'] or '(no heading)'}  "
            f"(source: {Path(r['source']).name}, score: {r['rrf_score']:.4f})\n"
            f"{r['text'][:600]}\n"
        )
    terminal_print(f"  \033[35m[RAG search] \"{query[:60]}\" -> {len(results)} hits\033[0m")
    return "\n".join(lines)


def run_index_documents(path: str = ".", patterns: str = "*.pdf") -> str:
    """Index documents into the knowledge base.

    If `path` is a single file, index that file directly.
    If `path` is a directory, scan it with the given glob patterns.
    """
    if not _rag_ready or _rag_bm25 is None or _rag_vector_store is None or _rag_embedder is None:
        return "Knowledge base not available. Set DASHSCOPE_API_KEY and restart."
    from pathlib import Path as P
    target = P(path)
    print(f"  \033[35m[RAG] indexing {path} ...\033[0m")
    try:
        if target.is_file():
            doc = rag_load_file(target)
            if not doc:
                return f"Could not load file: {path}"
            docs = [doc]
        else:
            pattern_list = [p.strip() for p in patterns.split(",")]
            docs = rag_load_directory(target, patterns=pattern_list)
        if not docs:
            return f"No documents found in {path} with patterns: {patterns}"
        chunks = rag_chunk_documents(docs)
        if not chunks:
            return f"No content extracted from {len(docs)} documents."
        _rag_bm25.add_chunks(chunks)
        _rag_vector_store.add_chunks(chunks, _rag_embedder)

        _rebuild_knowledge_index()

        return (
            f"Indexed {len(docs)} documents → {len(chunks)} chunks.\n"
            f"Vector store: {_rag_vector_store.count()} vectors.\n"
            f"BM25 index: {_rag_bm25.count()} documents."
        )
    except Exception as e:
...
BUILTIN_HANDLERS = {
    ...
    "index_documents": run_index_documents,
}


# ── RAG Knowledge Base ──

_rag_embedder: Embedder | None = None
_rag_bm25: BM25Index | None = None
_rag_vector_store: VectorStore | None = None
_rag_retriever: HybridRetriever | None = None
_rag_ready = False

RAG_VECTOR_PATH = WORKDIR / ".rag_vectors.json"
RAG_INDEX_DIR = WORKDIR / ".rag_index"
RAG_KNOWLEDGE_INDEX = RAG_INDEX_DIR / "KNOWLEDGE.md"
RAG_EMBED_API_KEY = os.getenv("DASHSCOPE_API_KEY", "")


def _extract_doc_title(chunks: list) -> str:
    """Extract a human-readable title from a document's chunks.

    Strategy: prefer the first chunk with a heading that looks like a title
    (long enough, not an author name or acronym). Fall back to the first
    non-empty text line.
    """
    for c in sorted(chunks, key=lambda c: c.chunk_index):
        h = c.heading.strip()
        # A good title: 10-120 chars, not just numbers/symbols, contains letters
        if 10 <= len(h) <= 120 and any(ch.isalpha() for ch in h):
            return h
    ...
    for c in sorted(chunks, key=lambda c: c.chunk_index):
        ...
        if 10 <= len(first_line) <= 120:
            return first_line
    return chunks[0].text[:80].strip() if chunks else ""


def _rebuild_knowledge_index():
    """Rebuild KNOWLEDGE.md from ALL documents currently in the vector store.

    Called after every index_documents so the system prompt always reflects
    the complete knowledge base, not just the most recently indexed batch.
    """
    RAG_INDEX_DIR.mkdir(exist_ok=True)
    if _rag_vector_store is None or _rag_vector_store.count() == 0:
        RAG_KNOWLEDGE_INDEX.write_text("", encoding="utf-8")
        return

    # Group chunks by source file
    doc_map: dict[str, dict] = {}
    for m in _rag_vector_store._metadata:
        source = m.get("source", "")
        name = Path(source).name if source else "unknown"
        if source not in doc_map:
            doc_map[source] = {"name": name, "chunks": 0, "sources": []}
        doc_map[source]["chunks"] += 1
        doc_map[source]["sources"].append(source)

    lines = []
    for source, info in doc_map.items():
        # Try to extract title from chunks of this document
        doc_chunks = [c for c in _rag_vector_store._metadata if c.get("source") == source]
        title = ""
        for c in sorted(doc_chunks, key=lambda c: c.get("chunk_index", 0)):
            h = c.get("heading", "")
            if 10 <= len(h) <= 120 and any(ch.isalpha() for ch in h):
                title = h
                break
    ...
    RAG_KNOWLEDGE_INDEX.write_text("\n".join(lines) + "\n" if lines else "", encoding="utf-8")


def _read_knowledge_index() -> str:
    """Read KNOWLEDGE.md for injection into the system prompt."""
    if not RAG_KNOWLEDGE_INDEX.exists():
        return ""
    text = RAG_KNOWLEDGE_INDEX.read_text(encoding="utf-8").strip()
    return text if text else ""


def init_rag():
    """Initialise RAG components. Safe to call multiple times."""
    global _rag_embedder, _rag_bm25, _rag_vector_store, _rag_retriever, _rag_ready

    if _rag_ready:
        return

    if not RAG_EMBED_API_KEY:
        print("  \033[33m[RAG] DASHSCOPE_API_KEY not set — knowledge search disabled\033[0m")
        return

    try:
        _rag_embedder = Embedder(api_key=RAG_EMBED_API_KEY)
        _rag_vector_store = VectorStore(uri=str(RAG_VECTOR_PATH))
        _rag_bm25 = BM25Index()

        # Rebuild BM25 from persisted vector store metadata
        # (BM25 is in-memory; vector store is on disk. On restart, rebuild.)
        count = _rag_vector_store.count()
        if count > 0 and _rag_vector_store._metadata:
            texts = [m.get("text", "") for m in _rag_vector_store._metadata]
            ids = [m.get("id", "") for m in _rag_vector_store._metadata]
            headings = [m.get("heading", "") for m in _rag_vector_store._metadata]
            sources = [m.get("source", "") for m in _rag_vector_store._metadata]
            _rag_bm25.add(texts, ids=ids, headings=headings, sources=sources)
            print(f"  \033[32m[RAG] ready — {count} vectors + BM25 loaded\033[0m")
        elif count > 0:
            print(f"  \033[32m[RAG] ready — {count} vectors loaded (BM25 empty)\033[0m")
        else:
            print(f"  \033[32m[RAG] ready — empty index\033[0m")

        _rag_retriever = HybridRetriever(_rag_bm25, _rag_vector_store, _rag_embedder)
        _rag_ready = True
    except Exception as e:
        print(f"  \033[31m[RAG] init failed: {e}\033[0m")
...
if __name__ == "__main__":
    CLI_ACTIVE = True
    print("s021: RAG-powered agent")
    print("Enter a question, press Enter to send. Type q to quit.\n")

    # Initialise RAG (reads DASHSCOPE_API_KEY from .env)
    init_rag()
